...
<div class="btn_left">
<input type="button" class="button" value="이전달" onclick="fn_index('be')"/>

</div>
<h3 id = "YM" style="text-align:center; margin:5px; padding-right:110px;"></h3>

<div class="btn_right">
<input type="button" class="button"  value="다음달" onclick="fn_index('ne')"/>
</div>
...
<div class="slide">
    <ul>
   <?
   //$index = "<script>documnet.write(index)</script>";
...
<script language="javascript">
	...
	//document.getElementById('fom_index').value = index;
	document.getElementById('fom_ym').value = month;
	
	function fninit() {
		console.log(YM);
		
		var y = document.getElementById('fom_ym').value;
		console.log(y);
		if(y != "") {
			document.getElementById("YM").innerHTML = y + "월의 작품";
		}
		else {
			document.getElementById("YM").innerHTML = YM;
	...
	document.getElementById("YM").innerHTML = YM;	 
      if(fg == 'be')   
	  {
		...
		/*YM = month + "월의 작품";
		document.getElementById("YM").innerHTML = month;*/
	  ...
	  console.log('value = ' + document.getElementById('fom_ym').value );
document.frm.submit();
   }
